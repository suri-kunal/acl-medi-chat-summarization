...
import wandb
from transformer_classifier import BertForSequenceClassificationMC
...
with open("TaskA-label2idx.json","w") as f:
    json.dump(label2idx,f,indent=2)
    
with open("TaskA-idx2label.json","w") as f:
...
tokenizer = AutoTokenizer.from_pretrained(
    code_config.MULTI_CLASS_MODEL_CHECKPOINT, do_lower_case=True, force_download=True
)
...
# %%
def create_dataset(
    input_ids_tensor, attention_mask_tensor, labels_tensor, full_sentence
):
...
def train_fn(model, train_dl, optimizer, scheduler, epoch):
    ...
    for train_step, batch in enumerate(train_dl):
        ...
        wandb.log(
            {
                "Batch/Training Step": train_step_new + 1,
...
def valid_fn(valid_dl, model, epoch=0, only_inference=False):
    ...
    for val_step, batch in enumerate(valid_dl):
        ...
        if only_inference is False:
            ...
            wandb.log(
                {
                    "Batch/Validation Step": val_step_new + 1,
                    "Batch/Validation Loss": loss.item(),
                }
            )
...
def train_valid_fn(num_classes, train_dl, valid_dl, split):
    ...
    random.seed(code_config.MULTI_CLASS_SEED)
    np.random.seed(code_config.MULTI_CLASS_SEED)
    torch.manual_seed(code_config.MULTI_CLASS_SEED)
    torch.cuda.manual_seed_all(code_config.MULTI_CLASS_SEED)
    
    model = BertForSequenceClassificationMC.from_pretrained(
        code_config.MULTI_CLASS_MODEL_CHECKPOINT,
        config=config,
        force_download=True,
    )
    model = model.to(device)
    ...
    optimizer = AdamW(
        ...
        eps=code_config.MULTI_CLASS_ADAMW_EPS,
        weight_decay=code_config.MULTI_CLASS_ADAMW_WEIGHT_DECAY,
    )

    total_steps = len(train_dl) * code_config.MULTI_CLASS_EPOCHS
    num_warmup_steps = int(code_config.MULTI_CLASS_WARM_UP_STEPS * total_steps)
    ...
    cfg.update(
        {
            ...
            "AdamW_eps": code_config.MULTI_CLASS_ADAMW_EPS,
            "AdamW_weight_decay": code_config.MULTI_CLASS_ADAMW_WEIGHT_DECAY,
            "warm_up_steps": code_config.MULTI_CLASS_WARM_UP_STEPS,
        }
    )
    ...
    for epoch_i in tqdm(range(code_config.MULTI_CLASS_EPOCHS)):
        model, avg_training_loss = train_fn(
            model, train_dl, optimizer, scheduler, epoch_i
        )
    ...
    new_model = BertForSequenceClassificationMC.from_pretrained(
        f"suryakiran786/{model_name}", config=config, force_download=True
    )
    ...
    log_validation_predictions(
        full_input_ids, truncated_input_ids, label_list, logits_list
    )
...
for split, (train_idx, valid_idx) in enumerate(
    skf.split(merge_df, y=merge_df["label"])
):
    ...
    (
        ...
        train_labels_tensor,
        train_full_sentences,
    ) = my_tokenizer(
        train_df["dialogue_wo_whitespaces"],
        train_df["label"],
        code_config.MULTI_CLASS_MAX_LENGTH,
    )
    (
    ...
    train_dl = create_dataloader(
        train_ds, RandomSampler(train_ds), code_config.MULTI_CLASS_BATCH_SIZE, 2
    )
    valid_dl = create_dataloader(
        valid_ds, SequentialSampler(valid_ds), 2 * code_config.MULTI_CLASS_BATCH_SIZE, 2
    )
