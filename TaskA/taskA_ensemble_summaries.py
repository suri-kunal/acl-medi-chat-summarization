...
def benchmarking(df,bm_split):
    ...
    for idx in tqdm(merge_df.index,desc="Summary"):
        ...
        merge_df.loc[idx,"best_summary"] = best_summary
        ...
        best_summary = summary_dict[best_model]
        merge_df.loc[idx,"faithful_summary"] = best_summary

    merge_df.to_csv(dest_path.joinpath(f"TaskA_{split}_best_summary_ensemble.csv"),index=False)
