...
def benchmarking(df,bm_split):
    ...
    merge_df.rename(mapper={"ID":"ID", \
                            "predicted_section_header":"section_header", \
                            "predicted_summary":"section_text"}, \
                    axis=1, \
                    inplace=True)
    merge_df.to_csv(f"TaskA_best_summary_benchmarck_{split}.csv",index=False)
...
if __name__ == "__main__":
    ...
    for split,(train_idx,valid_idx) in enumerate(skf.split(merge_df,y=merge_df["label"])):
        ...
        test_df.to_csv(f"TaskA_Best_Summary_Benchmark_GT_{split}.csv",index=False)
