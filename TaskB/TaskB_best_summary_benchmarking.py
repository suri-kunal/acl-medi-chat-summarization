...
def main(df,bm_split):
    ...
    _ = merge_df.pop("note")
    merge_df.rename(mapper={"encounter_id":"encounter_id","final_output":"note"},axis=1,inplace=True)
    merge_df = merge_df[["encounter_id","note"]]
    merge_df.to_csv(f"taskB_best_summary_max_{split}.csv",index=False)
...
if __name__ == "__main__":
    ...
    for split,(train_idx,valid_idx) in enumerate(kfold.split(merge_df)):
        ...
        valid_df.to_csv(f"taskB_best_summary_max_GT_{split}.csv",index=False)
        main(valid_df,split)
