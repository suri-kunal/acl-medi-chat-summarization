...
# %%
with open("taskb_summary_configuration_runtime.json","r") as f:
...
def main(df,bm_split):
    ...
    merge_df.to_csv(f"taskB_faithfulness_{split}.csv",index=False)
...
if __name__ == "__main__":
    ...
    for split,(train_idx,valid_idx) in enumerate(kfold.split(merge_df)):
        ...
        valid_df.to_csv(f"taskB_faithfulness_GT_{split}.csv",index=False)
        main(valid_df,split)
