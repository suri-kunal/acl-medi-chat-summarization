...
# %%
merge_ds = ds.Dataset.from_pandas(merge_df)
columns = merge_ds.column_names
print(columns)

# %%
processed_datasets = \
merge_ds.map(function=preprocess_function, \
                batched=True, \
...
# %%
processed_datasets.column_names
...
# %%
skf = StratifiedKFold(
    n_splits=code_config.TASKA_SUMMARY_N_SPLITS,
    shuffle=True,
...
#     new_model = AutoModelForSeq2SeqLM.from_pretrained(f"suryakiran786/{model_name}", \
#                                                   config=config, \
#                                                   force_download=True, \
#                                                   ignore_mismatched_sizes=True)
    
#     generated_tokens_list,reference_list,*_ = generate_summarization(new_model, \
#                                                                   valid_dataloader, \
#                                                                   epoch, \
#                                                                   only_inference=True)
    
#     if len(generated_tokens_list) != len(reference_list):
#         raise Exception("Length of generated_tokens_list is not equal to length of reference_list")    
    
#     all_scores = {}
#     for name, (scorer, kwargs, keys, save_keys) in scorers.items():
#         scores = scorer.compute(predictions=generated_tokens_list,references=reference_list, **kwargs)
#         for score_key, save_key in zip(keys, save_keys):
#             all_scores[save_key] = scores[score_key]
            
#     log_validation_data(generated_tokens_list,reference_list,all_scores,split)
#     for k,v in all_scores.items():
#         if isinstance(v,list) or isinstance(v,np.ndarray):
#             wandb.config.update({f"Best {k}":np.mean(v)})
#         else:
#             wandb.config.update({f"Best {k}":v})
