...
def main(filepath):
    ...
    combined_df["dialogue_wo_whitespaces"] = combined_df["dialogue"].apply(
        lambda x: re.sub(r"[\r\n\s]+", " ", x)
    )
    combined_df.reset_index(inplace=True)

    merge_df = pd.read_csv(filepath)
    ...
    for idx in tqdm(merge_df.index):
        ...
        # Collecting relevant dialogs for GPT3
        train_subset = combined_df.loc[combined_df["section_header"] == predicted_section_header]
        train_sample_indices = \
        np.random.choice(train_subset.index,size=min(3,train_subset.shape[0]),replace=False)
        if hasattr(train_sample_indices,"tolist"):
        ...
        try:
            predicted_summary = \
            openai_summary_generation(train_sample_indices,train_subset,dialogue_wo_whitespaces,predicted_section_header)
        except Exception as e:
            print(e)
        else:
            summary_dict["open-ai-model"] = predicted_summary.strip()
        
        best_model = "open-ai-model"
        best_summary = summary_dict["open-ai-model"]
        merge_df.loc[idx,"predicted_summary"] = best_summary
        merge_df.loc[idx,"model_name"] = best_model
        
    merge_df = merge_df[["ID","predicted_section_header","predicted_summary"]]
    merge_df.rename(mapper={"ID":"TestID", \
                            "predicted_section_header":"SystemOutput1", \
                            "predicted_summary":"SystemOutput2"}, \
                    axis=1, \
                    inplace=True)
    merge_df.to_csv("taskA_HealthMavericks_run3.csv",index=False)
