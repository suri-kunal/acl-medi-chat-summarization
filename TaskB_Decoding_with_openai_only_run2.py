...
def openai_summary_generation(sample_indices, \
    ...
    for row_idx, row in tqdm(sample_df.iterrows(),desc="OpenAI"):
...
def main(filename):
    ...
    train_df = train_df.apply(lambda x:add_section_divisions(x),axis=1)
    train_df = create_section_wise_dialogue(train_df)
    for section in SECTION_DIVISIONS:
        _ = train_df.pop(f"summary_{section}")
    ...
    for encounter_id in tqdm(merge_df["encounter_id"].unique(),desc="encounters"):
        ...
        for section, tokenizer_model_list in tokenizer_model_mapping.items():
            ...
            prompt = prompt_dictionary_by_section[section]

            test_dialogue = \
            merge_df.loc[merge_df["encounter_id"] == encounter_id,dialogue_column].item()
            
            train_subset = train_df[[dialogue_column,reference_column]].dropna()
            train_subset = train_subset.sample(min(5,train_subset.shape[0]))
            
            train_sample_indices = train_subset.index.tolist()
            
            predicted_summary = \
            openai_summary_generation(train_sample_indices, \
                                      train_subset, \
                                      dialogue_column, \
                                      reference_column, \
                                      test_dialogue, \
            ...
            summary_dict["open_ai"] = predicted_summary.strip()
    ...
    merge_df.to_csv("taskB_HealthMavericks_run2.csv",index=False)
